...
public class NumericUpDownValueEditor : PortValueEditor
{
    ...
    protected override void OnPortAttached(Port? port)
    {
        ...
        if (port.Metadata.TryGetValue("Numeric.Min", out var min) && min is double minVal) Min = minVal;
        if (port.Metadata.TryGetValue("Numeric.Max", out var max) && max is double maxVal) Max = maxVal;
        if (port.Metadata.TryGetValue("Numeric.Step", out var step) && step is double stepVal) Step = stepVal;
